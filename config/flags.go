...
func Flags() *pflag.FlagSet {
	...
	flags.String(flagKeyConfig, "travel-token-messenger-bot.yaml", "path to config file")
	...
	// DB config flags
	flags.String("db.path", "ttm-db", "Path to database dir.")

	// Partner plugin config flags
	flags.Bool("partner_plugin.enabled", false, "Enable or disable the partner plugin rpc client. It must be enabled if bot's ttm account supports at least one service.")
